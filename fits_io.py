...
def load_train_fits(path: str | Path) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]:
    path = Path(path)
    if not path.exists():
    ...
    with fits.open(path, memmap=True) as hdul:
        ...
        flux_clean = np.array(_require_hdu(hdul, "FLUX_CLEAN"), dtype=np.float32, copy=True)
        labels = _native_dataframe(_require_hdu(hdul, "LABELS"))
    return wavelength, flux, flux_clean, labels


def load_test_fits(path: str | Path) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]:
    path = Path(path)
    if not path.exists():
        raise FileNotFoundError(path)
